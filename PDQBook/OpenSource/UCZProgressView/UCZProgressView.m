...
- (void)setProgress:(CGFloat)progress animated:(BOOL)animated {
    ...
    self.progressLayer.strokeEnd = self.isUseArcAnimation ? progress : 0;
...
//        [self performFinishAnimation];
        [self performSelector:@selector(performFinishAnimation) withObject:nil afterDelay:0.2];
#endif
    }
...
- (void)performFinishAnimation {
    ...
    if (self.isUseArcAnimation) {
        ...
        /*
         ...
         [self.layer addAnimation:colorAnimation forKey:@"color"];
         */
        if (!self.blurEffect) {
            [self performSelector:@selector(recoverBackgroundColor) withObject:nil afterDelay:delay];
        }
