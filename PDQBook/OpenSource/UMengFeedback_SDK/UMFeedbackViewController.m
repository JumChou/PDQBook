...
#define UD_LoginUserID                  @"LoginUserID"
#define UD_LastUploadedWorkId           @"UD_LastUploadedWorkId"        // 最后一次上传的作品id
#define UD_LastUploadedWorkTitle        @"UD_LastUploadedWorkTitle"     // 最后一次上传的作品title

#define ChartCellIdentifier             @"chartCellId"
...
@property (nonatomic, strong) ChartCellFrame *cellFrames;
@end
...
//        forCellReuseIdentifier:@"postCellId"];
    [self.mTableView registerClass:[ChartCell class] forCellReuseIdentifier:ChartCellIdentifier];
    [self.mTableView registerClass:[UMChatTableViewCell class]
        forCellReuseIdentifier:@"chatCellId"];
    //        self.mTableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    self.mTableView.dataSource = self;
    self.mTableView.delegate = self;
    self.mTableView.backgroundColor = [UIColor colorWithHexString:@"e5e5e5"];
...
- (void)setupAirPPTContactInfo {
    NSString *userId = [[NSUserDefaults standardUserDefaults] valueForKey:UD_LoginUserID];
    NSString *workId = [[NSUserDefaults standardUserDefaults] valueForKey:UD_LastUploadedWorkId];
    NSString *workTitle = [[NSUserDefaults standardUserDefaults] valueForKey:UD_LastUploadedWorkTitle];
    
    NSString *plain = [NSString stringWithFormat:@"userId:%@     workId:%@     workTitle:%@",userId,workId,workTitle];
    NSString *email = @"";
    NSString *qq = @"";
    NSString *phone = @"";
    
    NSMutableDictionary *contactDict = [NSMutableDictionary dictionaryWithObjectsAndKeys:email,@"email",qq,@"qq",phone,@"phone",plain,@"plain", nil];
    NSMutableDictionary *contactInfo = [NSMutableDictionary dictionaryWithObjectsAndKeys:contactDict,@"contact",[NSNumber numberWithBool:NO],@"is_failed", nil];
    self.inputToolBar.contactInfo = contactInfo;
    
    if ([self.delegate respondsToSelector:@selector(updateUserInfo:)]) {
        [self.delegate updateUserInfo:contactInfo];
    }
}
...
- (CGSize)getViewSize:(UIInterfaceOrientation) interfaceOrientation {
    ...
    switch (interfaceOrientation) {
        ...
        case UIInterfaceOrientationPortraitUpsideDown: {
            ...
            viewHeight= [UIScreen mainScreen].bounds.size.height - 64;
            navHeight = 44;
            break;
        }
        case UIInterfaceOrientationLandscapeLeft:
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ChartCell *cell = [tableView dequeueReusableCellWithIdentifier:ChartCellIdentifier forIndexPath:indexPath];
    cell.cellFrame = [self chartCellFrameAtIndexPath:indexPath];
    return cell;
...
//                                                                forIndexPath:indexPath];
     ...
     */
//    
//    NSDictionary *dict = self.topicAndReplies[indexPath.row];
//    
//    
...
// 泡泡需要的数据源 与 友盟聊天的数据源进行关联
- (ChartCellFrame *)chartCellFrameAtIndexPath:(NSIndexPath *)indexPath {
    NSDictionary *chartInfo = self.topicAndReplies[indexPath.row];
    NSString *content = chartInfo[@"content"];
    NSString *icon = @"icon01.jpg";
    NSString *time = chartInfo[@"audio_length"];
    NSInteger type = kMessageFrom;
    if ([chartInfo[@"type"] isEqualToString:@"user_reply"]) {
        type = kMessageTo;
    }
    NSDictionary *cellInfo = @{@"content":content?content:@"",@"icon":icon,@"time":time?time:@"",@"type":@(type)};
    ChartCellFrame *cellFrame=[[ChartCellFrame alloc]init];
    ChartMessage *chartMessage=[[ChartMessage alloc]init];
    chartMessage.dict=cellInfo;
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return [[self chartCellFrameAtIndexPath:indexPath] cellHeight];
